...
sampleUI:newUI( { theme="darkgrey", title="Native Keyboard", showBuildNum=false } )
...
local phaseLabel = display.newText( mainGroup, "Input Phase:", 30-letterboxWidth, sampleUI.titleBarBottom+242, appFont, 17 )
...
local function createField( fieldType )
	...
	if ( fieldType == "Multi-Line" ) then
		field = native.newTextBox( display.contentCenterX, sampleUI.titleBarBottom+195, 260+(letterboxWidth*2), 60 )
		field.font = native.newFont( appFont, 17 )
		field.isEditable = true
	else
		field = native.newTextField( display.contentCenterX, sampleUI.titleBarBottom+195, 260+(letterboxWidth*2), 30 )
